...
// The 3D simulation pressure map. It REUSES the existing deck.gl 3D workload view
// (NYCWorkload3DDeck) and feeds it ABM stress-test pressure instead of live NYC 311
// metrics. The same extruded council-district polygons stand in as SYNTHETIC
// districts (POC modelling geometry — real Brampton ward geometry is not wired in),
// shaded + extruded by a selectable ABM pressure metric: case load, backlog, stale
// risk, or supervisor queue. This is planning simulation only — never live Brampton
...
const SIM_METRICS: SimMetricConfig[] = [
  { key: 'case_load', label: 'Case load', title: 'Generated case load', unit: 'cases' },
...
export default function SimulationPressureMap({
  ...
  // Bundled synthetic-district geometry — keyed by district number, labelled as a
  // SYNTHETIC district (not "Council District") because this is POC stand-in
  // geometry, not real Brampton ward boundaries.
  useEffect(() => {
    let active = true
    setGeomLoading(true)
    getNYCCouncilDistrictBoundariesCached()
      .then((districts) => {
        ...
        setUnits(
          districts.map((d) => ({
            ...
            label: `Synthetic District ${d.council_district}`,
            short: d.short_label,
      ...
      .catch((err: unknown) => {
        console.error('Failed to load synthetic district geometry:', err)
        if (active) setUnits([])
      })
      .finally(() => {
  ...
  return (
    <section aria-label="Simulation pressure map" className="card overflow-hidden">
      <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <h3 className="text-sm font-semibold text-navy-900">Simulation pressure map</h3>
          ...
          <p className="mt-1 max-w-2xl text-xs text-ink-subtle">
            Latest-run ABM pressure extruded by synthetic district. Drag to pan, right-drag to rotate.
          </p>
        </div>
      ...
      <div className="p-5">
        {loading || geomLoading ? (
          <div className="flex h-[320px] w-full items-center justify-center rounded-lg border border-dashed border-slate-300 bg-white/60 text-sm text-ink-subtle sm:h-[440px]">
            Loading simulation pressure map…
          </div>
        ) : !hasJoin ? (
          ...
          <figure className="relative rounded-lg bg-gradient-to-br from-slate-50 to-sky-50 p-4">
            ...
            {/* Legend — relative to the selected ABM metric across synthetic districts. */}
            <figcaption className="relative mt-3">
              <div className="flex items-baseline justify-between gap-2">
                ...
                <span className="text-[10px] text-ink-subtle">Low to high across synthetic districts</span>
              </div>
              <div
              ...
              <p className="mt-2 text-[11px] leading-relaxed text-ink-subtle">
                Height and colour are relative to the selected metric across synthetic districts; heights are scaled for
                ...
                Synthetic districts use NYC council-district benchmark geometry as POC stand-ins; real Brampton ward
                geometry is not yet wired in.
              </p>
            </figcaption>
          </figure>
        )}
